...
// Services
let refundService: WeArePlanet.api.RefundService = new WeArePlanet.api.RefundService(config);
let transactionCompletionService: WeArePlanet.api.TransactionCompletionService = new WeArePlanet.api.TransactionCompletionService(config);
 ...
 * Get refund payload
 */
function getRefundPayload(transaction: WeArePlanet.model.Transaction): WeArePlanet.model.RefundCreate {
    let refundPayload: WeArePlanet.model.RefundCreate = new WeArePlanet.model.RefundCreate();
    refundPayload.externalId = <string><any>transaction.id;
    refundPayload.type = WeArePlanet.model.RefundType.MERCHANT_INITIATED_ONLINE;
...
describe('RefundService', () => {
    ...
    describe('refund', () => {
        ...
        let refund: WeArePlanet.model.Refund;
        it('refund successful', () => {
            transactionService.create(config.space_id, getTransactionPayload())
                ...
                .then((response: { response: http.IncomingMessage, body: WeArePlanet.model.Transaction }) => {
                    ...
                    return transactionCompletionService.completeOffline(config.space_id, <number>transaction.id)
                        .then((response1: { response: http.IncomingMessage, body: WeArePlanet.model.TransactionCompletion }) => {
            ...
            }).done((response: { response: http.IncomingMessage, body: WeArePlanet.model.Refund }) => {
                refund = response.body;
                expect(refund.state).to.equal(WeArePlanet.model.RefundState.SUCCESSFUL);
            });
        });
    });
